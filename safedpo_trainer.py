...
SAFETY_PENALTY = 10
SAFEDPO_KEYS = ["better_is_unsafe", "worse_is_unsafe"]

@dataclass
class SafeDataCollatorForPreference(DataCollatorMixin):
    """
    ...
    def torch_call(self, examples: list[Union[list[int], Any, dict[str, Any]]]) -> dict[str, Any]:
        ...
        for example in examples:
            if SAFEDPO_KEYS[0] in example and SAFEDPO_KEYS[1] in example:
                safedpo_data = True
            break
        ...
        # do safedpo data
        if safedpo_data:
            for k in SAFEDPO_KEYS:
                output[k] = torch.BoolTensor(examples[k])

        return output


class SafeDPOTrainer(Trainer):
    r"""
    ...
    def __init__(
        ...
        safety_penalty: Optional[float] = 10,
    ):
        ...
        if data_collator is None:
            data_collator = SafeDataCollatorForPreference(pad_token_id=self.padding_value)
    ...
    def _prepare_dataset(
        ...
        with PartialState().local_main_process_first():
            ...
            # Pop safedpo specific dataset columns, then add them post-processing
            # Detect if we have the relevant columns, need: best_is_unsafe, worse_is_unsafe
            safedpo_dict = None
            best_is_unsafe = None
            worse_is_unsafe = None
            if SAFEDPO_KEYS[0] in dataset.features and SAFEDPO_KEYS[1] in dataset.features:
                safedpo_dict = {k: list(dataset[k]) for k in SAFEDPO_KEYS}
                dataset = dataset.remove_columns(SAFEDPO_KEYS)
            ...
            if safedpo_dict and isinstance(safedpo_dict, dict):
                for k, l in safedpo_dict.items():
                    dataset.add_column(k, l)
    ...
    def compute_ref_log_probs(self, batch: dict[str, torch.LongTensor, torch.BoolTensor]) -> dict:
    ...
    def concatenated_inputs(
        batch: dict[str, Union[list, torch.LongTensor, torch.BoolTensor]], padding_value: int
    ...
    def dpo_loss(
        ...
        better_is_unsafe: torch.BoolTensor | None,
        worse_is_unsafe = torch.BoolTensor | None,
        ...
        if self.f_divergence_type == FDivergenceType.ALPHA_DIVERGENCE.value:
            ...
            if better_is_unsafe and worse_is_unsafe:
                logratios = self.beta * (chosen_logps - ref_chosen_logps) + self.safety_penalty * better_is_unsafe
                ref_logratios = self.beta * (rejected_logps - ref_rejected_logps) + self.safety_penalty * worse_is_unsafe
                batch_size = better_is_unsafe.size(0)
                for i in range(batch_size):
                    if better_is_unsafe[i].item() > worse_is_unsafe[i].item(): 
                        tmp = logratios[i]
                        logratios[i] = ref_logratios[i]
                        ref_logratios[i] = tmp
                        better_is_unsafe[i], worse_is_unsafe[i] = worse_is_unsafe[i], better_is_unsafe[i]
            else:            
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor, torch.BoolTensor]]):
        ...
        output = {}
        # # safedpo data-centric processing
        # is_safedpo_data = True
        # for sdk in SAFEDPO_KEYS:
        #     if sdk not in batch:
        #         is_safedpo_data = False
        #     else:
        #         if not isinstance(batch[sdk], torch.BoolTensor):
        #             is_safedpo_data = False

        # if is_safedpo_data:
        #     for skd in SAFEDPO_KEYS:
        #         output[sdk] = batch[sdk]
    ...
    def get_batch_loss_metrics(
        self,
        model,
        batch: dict[str, Union[list, torch.LongTensor, torch.BoolTensor]],
        ...
        safedpo_data = {sdk: None for sdk in SAFEDPO_KEYS}
        is_safedpo_data = True
        for sdk in SAFEDPO_KEYS:
            if sdk not in batch:
                is_safedpo_data = False
        ...
        losses, chosen_rewards, rejected_rewards = self.dpo_loss(
            ...
            ref_chosen_logps,
            ref_rejected_logps,
            safedpo_data["better_is_unsafe"],
            safedpo_data["worse_is_unsafe"],
        )
    ...
    def generate_from_model_and_ref(self, model, batch: dict[str, torch.LongTensor, torch.BoolTensor]) -> tuple[str, str]:
